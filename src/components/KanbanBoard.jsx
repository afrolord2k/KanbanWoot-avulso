...
function KanbanBoard() {
  // DETECÇÃO DE ACCOUNT ID - Executado IMEDIATAMENTE
  const hostname = window.location.hostname;
  const subdomainMatch = hostname.match(/^(\d+)\.agentesintegrados\.com$/);
  const detectedAccountId = subdomainMatch ? subdomainMatch[1] : '1';
  ...
  // Atualiza título com Account ID
  const [accountId] = useState(detectedAccountId);
  useEffect(() => {
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      {/* DEBUG: Account ID detectado (escondido) */}
      <div style={{display: 'none'}} data-account-id={accountId}>Account: {accountId}</div>
